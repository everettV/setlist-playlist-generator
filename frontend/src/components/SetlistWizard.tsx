      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="flex flex-col h-[calc(90vh-8rem)]">
              ...
              <div className="space-y-3">
                {mockSetlists.map((setlist) => (
                  <div
                    ...
                    className="p-4 border-2 border-gray-200 rounded-lg hover:border-teal-500 hover:bg-gray-50 cursor-pointer transition-all"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-semibold text-gray-800">{setlist.tour}</h4>
                        <p className="text-sm text-gray-600 mt-1">{setlist.venue}</p>
                        <p className="text-sm text-gray-500">{setlist.city} • {setlist.date}</p>
                      </div>
                      <div className="text-right">
                        <span className="text-2xl font-bold text-teal-600">{setlist.songCount}</span>
          ...
          {step === 2 && selectedSetlist && (
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Preview Playlist</h3>
              <p className="text-sm text-gray-600 mb-4">
                {mockSongs.filter(s => s.found).length} of {mockSongs.length} songs available on {platformNames[platform]}
              </p>
              ...
              {/* Summary Info */}
              {mockSongs.filter(s => !s.found).length > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
                  <p className="text-sm text-amber-800">
                    <span className="font-medium">{mockSongs.filter(s => !s.found).length} songs</span> won't be available on {platformNames[platform]} and will be skipped.
                  </p>
                </div>
              )}
